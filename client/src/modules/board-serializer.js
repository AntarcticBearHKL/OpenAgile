...
const DROPPED_TASK_FIELDS = ['priority', 'dueDate', 'labels', 'subTasks', 'attachments', 'customFields', 'annotations', 'acceptanceCriteria', 'swimLaneOrder', 'swimlaneLabelGroup', 'swimlaneLabelId'];

const DROPPED_SETTINGS_FIELDS = ['swimLanesEnabled', 'swimLaneGroupBy', 'swimLaneLabelGroup', 'swimLaneOrder', 'swimLaneCollapsedKeys', 'swimLaneCellCollapsedKeys'];
 ...
 *
 * @param {{ board?, columns?, tasks?, settings? }} input
 * @returns {{ board, columns, tasks, settings, idMaps }}
 */
export function normalizeBoardModelIds({ board = null, columns = [], tasks = [], settings = null } = {}) {
  ...
  if (settings && typeof settings === 'object' && !Array.isArray(settings)) {
    ...
    for (const field of DROPPED_SETTINGS_FIELDS) {
  ...
  return {
    ...
    idMaps: { boardIdMap, columnIdMap, taskIdMap }
  };
}

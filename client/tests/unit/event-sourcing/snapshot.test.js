...
test('checkAndScheduleSnapshot schedules when snapshot age exceeds 14 days', async () => {
  ...
  const db = await openStore();
  await db.put('snapshots', { payload: { tasks: [], columns: [], settings: {}, globalSettings: {}, appliedEventIds: [], taskTombstones: [] }, hlc: makeHlc(1), at: oldAt }, 'board-a');
...
test('checkAndScheduleSnapshot does not delete events when a snapshot is taken', async () => {
  _setJitterForTesting(() => 0);
  const db = await openStore();
  for (let i = 0; i < SNAPSHOT_EVENT_THRESHOLD; i++) {
    await db.put('events', { ...makeEvent(`e${i}`, 'task.updated', makeHlc(i + 1)), synced: false });
  ...
  expect(await loadSnapshot('board-a')).not.toBeNull();
  const remaining = await db.getAll('events');
  expect(remaining.map(e => e.id)).toHaveLength(SNAPSHOT_EVENT_THRESHOLD);
});

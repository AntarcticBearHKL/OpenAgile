...
- **Pending hard-deletes queue:** Permanent deletes happen before sync runs. A global
  ...
  entry via the syncMap and hard-deletes the remote record, then clears the entry. Entries
  with no remote ID (task was created offline, never synced) are silently dropped.

- **Sync branching:** `pushBoardFull` must branch on the global `softDeleteEnabled` setting.
  Soft-delete on → upsert tasks with `deleted: true` to the remote store (no hard-delete during push).
  Soft-delete off → process `pendingHardDeletes` queue and hard-delete from the remote store.

- **Purge is cross-board:** The purge operation in Settings hard-deletes all soft-deleted tasks
  across all boards from both IDB and the remote store (immediately if online; via queue if offline).

...
IndexedDB are reducer-maintained **projections** rebuilt from the event stream. A remote event store
is an optional fan-out, not a dependency — offline-only-forever remains a first-class mode.

This replaces the previous whole-record last-write-wins (LWW) push/pull sync (PR #89), in which a
device pushed entire board records to a remote store and pulled them back. The dominant failure of that
...
a single user's devices are rare; the real problem is ordering, not field-level merge.
...
- **A dumb event store.** The remote store holds an extended `events` collection (`hlc`, `scope`,
  ...
  not a relation), so board-scoped events validate.
- **Client-side snapshots (W1).** Snapshots are computed on the client and uploaded (race-on-write);
  old events are GC'd locally and on the remote store after each snapshot. Chosen over a server-side
  projector (W3) for simplicity.
- **Realtime via SSE.** A Server-Sent-Events subscription applies remote events live; a
  launch/reconnect **catch-up pull** converges a just-opened device. The remote store is optional —
  without an account nothing leaves the browser.
...
- **Header sync-state indicator (#115)** replaces the manual Sync button; it derives `Live` /
  ...
  status.
- **Legacy per-entity collections** are locked read-only and scheduled for removal after a 30-day quiet
  period (issue #116).
